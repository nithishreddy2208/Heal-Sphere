...
const PatientAppointments = () => {
  ...
  const openMedicalRecords = async (appointment) => {
    ...
    try {
      ...
      if (!response.ok) {
        setMedicalError("No medical reports.");
        setMedicalRecords([]);
        return;
      }
      ...
      const filtered = doctorId
        ? list.filter((p) => {
            const pid = p?.doctorId?._id || p?.doctorId;
            return pid?.toString?.() === doctorId?.toString?.();
          })
        : list;
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      if (user) fetchPatientAppointments();
    }, 60000);
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    ...
                    <button
                      onClick={() => openMedicalRecords(appointment)}
          ...
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-5">
              {medicalLoading ? (
                ...
                <div className="text-gray-600">{medicalError}</div>
              ) : medicalRecords.length === 0 ? (
                <div className="text-gray-600">No medical reports.</div>
              ) : (
                <div className="space-y-4">
                  {medicalRecords.map((p) => (
                    <div
                      key={p._id}
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          <p className="text-gray-900 font-semibold break-words">
                            {p.prescriptionName || "Medical Report"}
                          </p>
                          {p.issuedOn && (
                            <p className="text-sm text-gray-600 mt-1">
                              Issued on: {p.issuedOn}
                            </p>
                          )}
                          {p.cause && (
                        ...
                        <div className="mt-3">
                          ...
                          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {p.medicines.map((m, idx) => (
                              <div
                                key={`${p._id}-m-${idx}`}
                                className="text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-lg p-2"
                              >
                                {m}
                                {Array.isArray(p.dosages) && p.dosages[idx] ? (
                                  <span className="text-gray-500"> — {p.dosages[idx]}</span>
                                ) : null}
                              </div>
                            ))}
                          </div>
