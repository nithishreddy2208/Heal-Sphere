...
const bookAppointment = async (req, res) => {
  try {
    ...
    const blockedStart = moment(startIST).subtract(1, "hours").toDate();
    const blockedEnd = moment(startIST).add(1, "hours").toDate();

    const conflictAppointment = await AppointmentModel.findOne({
      doctorId: doctor.userId,
      status: { $in: ["Pending", "PendingDoctor", "Accepted"] },
    ...
    if (conflictAppointment) {
      return res.status(400).json({
        message:
          "Doctor is busy for the selected time. Please choose another slot.",
      });
    }
...
const upDateAppointment = async (req, res) => {
  try {
    ...
    if (status === "Accepted") {
      appointment.status = "PendingDoctor";
    } else {
      appointment.status = status;
    }
    await appointment.save();
    ...
    const patientMessage =
      status === "Accepted"
        ? `Your appointment with ${doctor.name} is Pending Doctor Approval`
        : `Your appointment with ${doctor.name} is ${status}`;
    ...
    if (status === "Accepted") {
      const doctorNotification = new NotificationModel({
        ...
        doctorId: appointment.doctorId._id,
        appointmentId: appointment._id,
        message: `New appointment request from ${patient?.userName || "Patient"}`,
      });
...
const doctorUpdateAppointmentStatus = async (req, res) => {
  try {
    const { appointMentId, status } = req.body;
    ...
    if (appointment.doctorId.userId.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "Not authorized" });
    }

    if (appointment.status !== "PendingDoctor") {
      return res.status(400).json({ message: "Appointment is not pending doctor approval" });
    }
    ...
    const doctor = appointment.doctorId;
    const patient = await UserModel.findById(appointment.patientId);
    const patientMessage = `Your appointment with ${doctor.name} is ${status}`;

    const patientNotification = await NotificationModel.findOne({
      userId: appointment.patientId,
      doctorId: appointment.doctorId._id,
      appointmentId: appointment._id,
    });
    ...
    if (patientNotification) {
      patientNotification.message = patientMessage;
      await patientNotification.save();
    }

    const doctorNotification = await NotificationModel.findOne({
      userId: req.user._id,
      doctorId: appointment.doctorId._id,
      appointmentId: appointment._id,
    });
    ...
    if (doctorNotification) {
      doctorNotification.markAsRead = true;
      await doctorNotification.save();
    }
